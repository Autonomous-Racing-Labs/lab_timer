...
Freenove_ESP32_WS2812::~Freenove_ESP32_WS2812()
{
	delete led_data;
	led_data = NULL;
}
...
{	
	...
	for(int i=0;i<ledCounts;i++)
	{
		for (int bit = 0; bit < 32; bit++) {
			led_data[i*32+bit].level0 = 1;
			led_data[i*32+bit].duration0 = 4;
			led_data[i*32+bit].level1 = 0;
			led_data[i*32+bit].duration1 = 8;
		}
	}
...
esp_err_t Freenove_ESP32_WS2812::set_pixel(int index, uint8_t  r, uint8_t  g, uint8_t  b)
{
	...
	for (int bit = 0; bit < 32; bit++) {
		if (color & (1 << (31-bit))) {
			led_data[index*32+bit].level0 = 1;
			led_data[index*32+bit].duration0 = 8;
			led_data[index*32+bit].level1 = 0;
			led_data[index*32+bit].duration1 = 4;
		} else {
			led_data[index*32+bit].level0 = 1;
			led_data[index*32+bit].duration0 = 4;
			led_data[index*32+bit].level1 = 0;
			led_data[index*32+bit].duration1 = 8;
		}
	}
	return ESP_OK;
}

esp_err_t  Freenove_ESP32_WS2812::random() {
